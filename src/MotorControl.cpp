...
void moveForwardPID(int _leftSpeed, int _rightSpeed, bool withOutLine, bool lineTracking) {
  ...
  // Apply correction: if line is to the right (positive error), turn right
  // Turn right = slow right wheel, speed up left wheel
  _leftSpeed += correction;       // If positive error (line right), speed up left
  _rightSpeed -= correction;      // If positive error (line right), slow right

  // Keep speeds within valid PWM range (minimum 50 to prevent stalling)
  _leftSpeed = constrain(_leftSpeed, 50, 255);
  _rightSpeed = constrain(_rightSpeed, 50, 255);
